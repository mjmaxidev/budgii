...
| Push notifications          | Backend stores per-user device tokens, has a log push provider, filters by type/quiet hours, and Preferences requests browser permission | Real APNs/FCM provider, native token capture, backend scheduler deployment                                    |
...
| Notifications          | In-app generated notifications, read state, per-user device token registration, push dispatch idempotency, type preferences, and quiet hours                                                                              | Actual APNs/FCM delivery, native token capture, scheduler deployment, weekly summary generation/sending                                                                                  |
...
| Recurring Transactions | Synced config + backend apply-on-hydration, visible schedule status, manual preview/apply controls, and backend worker/CLI | Needs production scheduler deployment                                     |
...
- Add APNs/FCM delivery, native token capture, and production scheduler deployment.
