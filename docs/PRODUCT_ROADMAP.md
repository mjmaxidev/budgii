...
| ---------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Recurring transactions | Config UI, sync, backend apply, idempotent due expense creation              | Background scheduler/cron, `startDate`, enabled/disabled toggle, next due/last applied UI, manual “apply due now”/preview, user-facing failure status                                    |
...
| Preferences            | Currency/language/notification settings save/sync                            | Currency conversion/exchange rates not implemented; language switch does not provide full i18n; push toggle does not request OS permission                                               |
...
| Preferences            | Synced settings                                                     | Push/language/currency are settings only, not full provider/i18n features |
| Income Tracking        | Synced JSONB config/data                                            | Not normalized                                                            |
| Recurring Transactions | Synced config + backend apply-on-hydration                          | Needs scheduler-grade controls/status                                     |
...
- Add recurring `startDate`, `enabled`, `lastAppliedAt`, and computed `nextDueDate`.
- Add UI status and preview/apply-now controls.
- Add a backend scheduled job/worker for recurring expenses.
