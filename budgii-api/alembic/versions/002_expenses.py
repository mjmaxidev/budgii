...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("tag_ids", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("notes", sa.Text(), nullable=True),
        sa.Column("receipt_upload_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("receipt_uploads.id", ondelete="SET NULL"), nullable=True),
